...
class EDACPolicy(BasePolicy):
    ...
    def learn(self, batch: Dict) -> Dict:
        ...
        log_probs = self._sum_logprob(log_probs)   
        ...
        if self._is_auto_alpha:
            log_probs_adj = log_probs.detach() + self._target_entropy
            alpha_loss = -(self._log_alpha * log_probs_adj).mean()
        ...
        if self._max_q_backup:
            ...
            with torch.no_grad():
                ...
                next_log_probs = self._sum_logprob(next_log_probs)
        ...
        result =  {
            ...
            "loss/critics": float(critics_loss.item())
        }

        if self._is_auto_alpha:
            result["loss/alpha"] = float(alpha_loss.item())
            result["alpha"] = float(self._alpha.item())
